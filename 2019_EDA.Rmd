...
# Hands-on 2 Distribution of data (10 min)
...
# Hands-on 5 Mean, error bars and fit line (20 min) 
...
# Hands-on 6 Scatter plot with regression line (10 min)
...
Very nice tutorial here <http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html>

Select a random sample of size 1000

```{r}
library(ggplot2)
dim(diamonds)
diam2 <-  diamonds[sample(1:nrow(diamonds), 1000, replace = TRUE),]
```

From the dataset diam2, we select variables carat and price and cut. We plot scatter diagram and smooth plot together. 

```{r}
ggplot(diam2, aes(x=carat, y=price, color=cut)) + geom_point() + geom_smooth()
```

This is a scatterplot without the smooth plot

```{r}
gg <- ggplot(diam2, aes(x = carat, y = price, color = cut)) + 
  geom_point() + 
  labs(title = "Scatterplot using Price and Carat",
                      x = "Carat", y = "Price")  
print(gg)
```
